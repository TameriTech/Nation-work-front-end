// app/(protected)/dashboard/admin/users/[userId]/error.tsx
"use client";

import { AlertCircle, RefreshCw, Home, ArrowLeft, User } from "lucide-react";
...
export default function UserDetailError({ error, userId, onRetry }: UserDetailErrorProps) {
  ...
  const getErrorMessage = () => {
    ...
    if (error.message.includes("404") || error.message.includes("introuvable")) {
      return `L'utilisateur avec l'ID ${userId} est introuvable.`;
    }
    ...
    return error.message || "Une erreur est survenue lors du chargement de l'utilisateur";
  };
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl overflow-hidden border border-gray-200 dark:border-slate-700">
          ...
          <div className="bg-red-500 dark:bg-red-600 p-6 text-center">
            <div className="inline-flex p-3 bg-white dark:bg-slate-800 rounded-full">
              <User className="w-12 h-12 text-red-500 dark:text-red-400" />
            </div>
          </div>
