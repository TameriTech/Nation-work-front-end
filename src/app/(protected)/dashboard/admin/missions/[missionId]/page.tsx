...
// Timeline
const Timeline = ({ service }: { service: ServiceDetail }) => {
  const events = [
    {
      status: "published",
...
export default function ServiceDetailPage() {
  ...
  // Hook personnalisé
  const {
    useAdminServiceDetails,
    changeStatus,
    isChangingStatus,
    deleteService,
    isDeleting,
  } = useServices({ mode: "admin" });
  ...
  const {
    ...
    error,
    refetch,
  } = useAdminServiceDetails(serviceId);
  ...
  const handleUpdateStatus = async (newStatus: ServiceStatus) => {
    ...
    if (confirm(`Changer le statut en ${newStatus} ?`)) {
      try {
        await changeStatus({ id: service.id, data: { status: newStatus } });
  ...
  const handleCancel = async (data: CancelFormData) => {
    ...
    try {
      await changeStatus({
        ...
        data: {status: "cancelled" as ServiceStatus,
        reason: data.reason,
        notify: data.notify,}
      });
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              onClick={handleEdit}
              className="hidden px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-gray-100 flex items-center transition"
            >
            ...
            {service.status !== "cancelled" &&
              service.status !== "completed" && (
                <button
                  onClick={() => handleUpdateStatus("cancelled" as ServiceStatus)}
                  disabled={isChangingStatus}
                  className="hidden  px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 flexx items-center transition disabled:opacity-50"
                >
                  {isChangingStatus ? (
                    <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                  ) : (
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap gap-4">
                ...
                <div className="flex items-center text-sm text-gray-500 dark:text-gray-400">
                  ...
                  {service.candidatures_count || 0} candidature(s)
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {service.questions && service.questions.length > 0 && (
              <InfoSection title="Questions et réponses" icon={MessageSquare}>
                <div className="space-y-4">
                  {service.questions.map((question, index) => (
                    <div key={index} className="border-b border-gray-200 dark:border-slate-700 last:border-0 pb-4 last:pb-0">
                      <p className="font-medium text-gray-900 dark:text-white mb-1">
                        {question.question}
                      </p>
                      <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
                        Par {question.user_name} le{" "}
                      ...
                      {question.answer && (
                        <div className="mt-2 pl-4 border-l-2 border-blue-500">
                          ...
                          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                            Répondu le{" "}
                            {new Date(question.answered_at!).toLocaleDateString("fr-FR")}
                          </p>
                        </div>
                      )}
                    </div>
            ...
            {service.rating && (
              <InfoSection title="Évaluation" icon={Star}>
                ...
                <p className="text-gray-700 dark:text-gray-300 italic">
                  "{service.rating.comment || "Aucun commentaire fourni."}"
                </p>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                  {new Date(service.rating.created_at).toLocaleDateString("fr-FR")}
                </p>
              </InfoSection>
            )}
          ...
          <div className="space-y-6">
            ...
            <InfoSection title="Client" icon={User}>
              <div className="flex items-center mb-4">
                ...
                <div className="ml-4">
                  <p className="text-lg font-medium text-gray-900 dark:text-white">
                    {service.client.full_name || service.client.username}
                  </p>
                  <Link
              ...
              <InfoRow
                label="Email"
                value={service.client.email || "Non renseigné"}
                icon={Mail}
              />
              <InfoRow
                label="Téléphone"
                value={service.client.phone_number || "Non renseigné"}
                icon={Phone}
              />
            </InfoSection>

            {/* provider */}
            <InfoSection title="provider" icon={User}>
              {service.provider ? (
                <>
                  <div className="flex items-center mb-4">
                    <div className="flex-shrink-0 h-12 w-12">
                    ...
                    <div className="ml-4">
                      <p className="text-lg font-medium text-gray-900 dark:text-white">
                        {service.provider?.full_name || service.provider?.username}
                      </p>
                      {service.provider?.average_rating && (
                        <div className="flex items-center text-sm text-gray-500 dark:text-gray-400">
                          <Star className="w-4 h-4 mr-1 text-yellow-400 fill-current" />
                          {service.provider.average_rating}/5
                        </div>
                      )}
                      <Link
                        href={`/dashboard/admin/users/${service.provider?.id}`}
                        className="text-sm text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300"
                      >
                        Voir le profil
                      </Link>
                  ...
                  <InfoRow
                    label="Email"
                    value={service.provider?.email || "Non renseigné"}
                    icon={Mail}
                  />
                </>
              ) : (
            ...
            <InfoSection title="Détails pratiques" icon={MapPin}>
              <InfoRow
                label="Date et heure"
                value={`${new Date(service.scheduled_date).toLocaleDateString("fr-FR")}`}
                icon={Calendar}
              />
              <InfoRow
                label="Duree"
                value={String(service.duration_value) + " " + service.duration_unit} icon={Calendar}/>
              {service.duration && (
                <InfoRow label="Durée" value={service.duration} icon={Clock} />
              )}
              <InfoRow label="Adresse" value={service.location.address} icon={MapPin} />
              {service.location.city && (
                <InfoRow label="Ville" value={service.location.city} icon={MapPin} />
              )}
            </InfoSection>
